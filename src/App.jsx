...
function mergeAllStats(rMap, roster) {
  ...
  const sm = {Goals:"goals",Assists:"assists","Shot on Goal":"shotsOnGoal","Shot Missed":"shotsMissed",Groundballs:"groundballs","Turnovers Caused":"cto",Penalty:"penalties"};
  Object.entries(sm).forEach(([sT,f]) => { const mx={}; Object.values(rMap).forEach((d)=>{const ct={};(d.stats||[]).forEach((s)=>{if(s.type===sT&&s.player){const k=s.player+"-"+s.number;ct[k]=(ct[k]||0)+1;}});Object.entries(ct).forEach(([k,v])=>{mx[k]=Math.max(mx[k]||0,v);});}); Object.entries(mx).forEach(([k,v])=>{if(ps[k])ps[k][f]=v;}); });
  const mxFT={},mxFW={};
  ...
  if(screen==="home"){
    ...
    return(
      <div className="min-h-screen bg-gradient-to-br from-red-600 via-red-500 to-yellow-500 p-4"><div className="max-w-md mx-auto"><div className="bg-white rounded-lg shadow-2xl p-6 mt-4 border-4 border-black relative">
        ...
        <div className="mb-4"><label className="block text-sm font-bold text-gray-700 mb-1">Your Name</label><input type="text" value={formName} onChange={(e)=>setFormName(e.target.value)} className="w-full px-4 py-2 border-2 border-black rounded-lg" placeholder="Enter your name"/></div>
        ...
        {actG.length>0&&<div className="mt-6"><h3 className="font-black text-gray-800 mb-2 text-sm">ACTIVE GAMES</h3>{actG.map((g)=>(
          <div key={g.id} className="mb-2 bg-green-50 border-2 border-green-400 rounded-lg p-3">
            ...
            <div className="flex gap-2 mt-2">
              <Btn onClick={()=>{if(!formName){alert("Please enter your name first");return;}setCurGameId(g.id);setCurGame(g);setRecorderName(formName);const gl=g.roster?g.roster.filter(p=>p.position==="Goalie"):roster.filter(p=>p.position==="Goalie");setActiveGoalie(gl.length?gl[0].name:"");setCurrentPeriod(g.periods[0]);(async()=>{const ex=await sGet("game:"+g.id+":stats:"+formName);if(ex){setStats(ex.stats||[]);setGoalSeq(ex.goalSeq||0);setGaSeq(ex.gaSeq||0);}else{setStats([]);setGoalSeq(0);setGaSeq(0);}setScreen("tracking");})();}} cls="flex-1 py-1 bg-green-600 text-white rounded text-xs font-black border border-black">JOIN</Btn>
  ...
  if(screen==="newgame"){return(
    <div className="min-h-screen bg-gradient-to-br from-red-600 via-red-500 to-yellow-500 p-4"><div className="max-w-md mx-auto"><div className="bg-white rounded-lg shadow-2xl p-6 mt-4 border-4 border-black">
      ...
      <div className="space-y-4">
        ...
        <div><label className="block text-sm font-bold text-gray-700 mb-1">Date</label><input type="date" value={ngDate} onChange={(e)=>setNgDate(e.target.value)} className="w-full px-4 py-2 border-2 border-black rounded-lg text-sm" style={{maxWidth:"100%"}}/></div>
